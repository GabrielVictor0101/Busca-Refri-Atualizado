...
const CreateEmployee = ({ navigation, route }) => {
  const getDetails = (type) => {
    if (route.params) {
      switch (type) {
        ...
        case 'marca':
          return route.params.marca;
        case 'qtdLitro':
          return route.params.qtdLitro;
        case 'garrafa':
          return route.params.garrafa;
        case 'foto':
  ...
  const [marca, setMarca] = useState(getDetails('marca'));
  const [qtdLitro, setQtdLitro] = useState(getDetails('qtdLitro'));
  const [garrafa, setGarrafa] = useState(getDetails('garrafa'));
  const [foto, setFoto] = useState(getDetails('foto'));
  const [modal, setModal] = useState(false);
  const [enableshift, setenableShift] = useState(false);

  const submitData = () => {
    fetch('http://192.168.1.6:3000/send-data', {
      method: 'post',
      headers: {
  ...
  const updateDetails = () => {
    fetch('http://192.168.1.6:3000/update', {
      method: 'post',
      headers: {
        'Content-Type': 'application/json',
      },
    ...
    >
      <View>
        ...
        <TextInput
          ...
          mode="outlined"
          onChangeText={(text) => setQtdLitro(text)}
        />
        ...
        <TextInput
          ...
          mode="outlined"
          onChangeText={(text) => setGarrafa(text)}
        />
        <Button
          style={styles.inputStyle}
